...
class CanvasWindow : public AbstractWindow
    ...
    CanvasWindow (int id, int x, int y, int width, int heigh, const char* imgTxt, COLORREF Color);
...
int main()
    ...
    CanvasWindow Canvas  (1, 110,   0, 690, 690, NULL, NULL);
...
int CanvasWindow::Action (int MouseX, int MouseY)
    ...
    txSetFillColor (Color_, Invis_);
    txSetColor (ColorNew, 1, Invis_);

    txEllipse (MouseX-5, MouseY-5, MouseX+5, MouseY+5, Invis_);

    printf ("color = %06x\n", Color_);

    txBitBlt (txDC(), 110, 0, 690, 690, Invis_);
...
CanvasWindow::CanvasWindow (int id, int x, int y, int width, int heigh, const char* imgTxt, COLORREF Color) :
    AbstractWindow (id, x, y, width, heigh, imgTxt, Color, 1),
    Invis_ (txCreateDIBSection (width, heigh))
    {}
